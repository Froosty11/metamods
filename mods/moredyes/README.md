...
- The beam scrolls with no packets: each texture is a **4-frame** vertical strip with a `.mcmeta`
  animation, generated by `runDatagen` from vanilla's `entity/beacon/beacon_beam` (as
  `textures/block/beacon_beam_{core,glow}_{1,2,4,8,16}.png`, under `block/` so the blocks atlas picks
  them up). Frame *f* is the tile rolled up *f*/4 of a tile and a frame lasts a tick, so the pattern
  travels **one block every four ticks**; vanilla's `v` offset moves 0.2 of a tile a tick, one block
  every five, and `interpolate` makes the four steps a continuous crawl rather than a stutter. A
  block model cannot tile a face's UV, which is why there is a texture per segment height: a single
  strip stretched over a segment showed the whole pattern once however tall the segment was — 16×
  too coarse on a full one, and the scroll crawled with it. All ten are 16 px wide (16 × 1024 at the
  largest), so together they cost less atlas area than one 128 × 128 block texture.
- The beam **spins**, like vanilla's. `BeaconRenderer` turns the whole beam by
  `animationTime * 2.25 - 45` degrees about `Axis.YP`, i.e. 2.25° a tick. We cannot send a packet a
  tick, so every 20-tick refresh each shown element gets **+45° of left rotation** with
  `interpolation_duration` 20 and a start-interpolation flag, and the client slerps across exactly
  the second it takes for the next one to arrive: one small metadata update per element per second.
...
itself looks right; only the light it casts is missing. Vanilla also offsets its glow part by an
extra 45° from its core (the core is a diamond, the glow a box); ours are both boxes in step.
Vanilla scales the core's texture 2.5 repeats a block, against one for the glow, to keep its texels
square on a narrower column; ours is one repeat a block on both.
