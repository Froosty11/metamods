...
Status: **spike**, throwaway-grade, `metacraft.moredyes.beacon`. It compiles, loads and runs, but
nobody has looked at it with a real client yet.
...
- **Near players** (default 48 blocks, horizontal) are sent a **barrier** where the beacon is and get
  ...
  two `ItemDisplayElement`s per beam section — an opaque core (vanilla's `SOLID_BEAM_RADIUS` 0.2) and
  a translucent glow (`BEAM_GLOW_RADIUS` 0.25), scaled in Y to the section height. Each carries a
  `dyed_color` component that an item-model definition's `minecraft:dye` tint multiplies into the
  beam texture — block displays cannot be tinted to an arbitrary RGB, item displays can. The beacon
  still works: the barrier is only what the client is shown, the server block is the real beacon.
...
partial alpha actually renders translucent on an item display, whether a tall display gets culled
despite `setDisplaySize(0, 0)` and `setViewRange(4)`, and whether the stretched (not tiled) beam
texture on a long section looks acceptable. Known rough edges: panes in the column contribute their
colour to the walk but are not swapped for far players (glass only); a section's texture is stretched
over its whole height rather than tiled per block; and on a fresh chunk send a player briefly sees
the default look until the next 20-tick sweep, because `PolymerBlock#getPolymerBlockState` has no
block position (Polymer maps whole chunk palettes with it), so the per-player exceptions have to be
pushed as explicit block updates from the holder, which does know the positions.
